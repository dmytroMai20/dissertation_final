...
class ddpm(torch.nn.Module):
    ...
    def train_step(self,imgs, optimizer):
        optimizer.zero_grad()
        self.train()
        t = torch.randint(0, self.timesteps, (imgs.shape[0],), device=self.device).long()
    ...
    def p_sample_loop(self, z=None):
        ...
        if z == None:
            img = torch.randn((self.batch_size, self.channels, self.res, self.res), device=self.device)
        else:
            img = z
        #imgs = []
