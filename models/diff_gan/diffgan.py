...
class diffusiongan(torch.nn.Module):
    def __init__(self, config):
        ...
        self.generator = Generator(int(math.log2(config.res)),config.dim_w).to(config.device)
        self.discriminator = Discriminator(int(math.log2(config.res))).to(config.device)
        self.mapping_net = MappingMLP(config.dim_w, config.mappingnet_layers).to(config.device)
    ...
    def train_step(self, real_images, d_optim, g_optim, mlp_optim, batch_idx, epoch, num_batches):
        ...
        if (batch_idx+1) % self.t_update_interval == 0:
            adjust = np.sign(calculate_r_d(real_output).item() - self.d_target) * (self.batch_size * self.t_update_interval) / (self.update_kimg * 1000)
