...
from models.stylegan2.stylegan2 import stylegan2
from torch.optim import Adam
from models.stylegan2.ema import EMA
import torch
...
class StyleGAN2Trainer():
    ...
    def __init__(self, config, dataloader):
        ...
        self.g_optim = Adam(self.model.generator.parameters(),
                            lr=config.lr,
                            betas=(0.0, 0.99))
        self.d_optim = Adam(self.model.discriminator.parameters(),
                            lr=config.lr,
                            betas=(0.0, 0.99))
        self.mlp_optim = Adam(self.model.mapping_net.parameters(),
    ...
    def train(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_idx, (imgs, _) in enumerate(tqdm(self.data_loader)):
                ...
                loss = self.model.train_step(imgs, self.d_optim, self.g_optim,
                                             self.mlp_optim, batch_idx, epoch,
                                             num_batches)
            ...
            save_generated_images(
                generated_images,
                f'stylegan2_{self.config.dataset}_{str(self.config.res)}/epoch_{str(epoch)}'
            )
